...
void iree_hal_amdgpu_kernel_dispatch(
  ...
  const uint64_t packet_id =
      iree_hsa_queue_add_write_index_screlease(libhsa, queue, 1);
  while ((packet_id - iree_hsa_queue_load_read_index_scacquire(
                          libhsa, queue)) >= queue->size) {
    iree_thread_yield();
  }
  ...
  iree_hsa_signal_store_relaxed(libhsa, queue->doorbell_signal, packet_id);
}
...
static iree_status_t iree_hal_amdgpu_allocate_scheduler(
  ...
  *out_scheduler = NULL;
  ...
  if (iree_status_is_ok(status)) {
    *out_scheduler = (iree_hal_amdgpu_device_queue_scheduler_t*)slab;
    *out_kernargs =
...
iree_status_t iree_hal_amdgpu_queue_initialize(
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_amdgpu_allocate_scheduler(
        ...
        fine_memory_pool, &out_queue->scheduler, &scheduler_kernargs);
    out_queue->scheduler_ptrs.control_kernargs = scheduler_kernargs;
  }
...
static void iree_hal_amdgpu_queue_request_tick(
  ...
  // mailbox. The returned value prior to the operation lets us know if a tick
  ...
  if (iree_amdgpu_scoped_atomic_fetch_or(
          &queue->scheduler->tick_action_set, action_set,
  ...
  IREE_TRACE_ZONE_BEGIN(z0);
  IREE_TRACE_ZONE_APPEND_VALUE_I64(z0, action_set);
...
static void iree_hal_amdgpu_queue_commit_entry(
  ...
  const uint64_t entry_index = iree_atomic_fetch_add(
      &queue->scheduler->mailbox.write_index, 1, iree_memory_order_release);
  while ((entry_index - iree_atomic_load(&queue->scheduler->mailbox.read_index,
                                         iree_memory_order_acquire)) >=
         IREE_ARRAYSIZE(queue->scheduler->mailbox.entries)) {
    iree_thread_yield();
  }
  const uint64_t entry_mask =
      IREE_ARRAYSIZE(queue->scheduler->mailbox.entries) - 1;
  IREE_AMDGPU_DEVICE_PTR iree_atomic_uint64_t* entry_ptr =
      (iree_atomic_uint64_t*)&queue->scheduler->mailbox
          .entries[entry_index & entry_mask];
...
void iree_hal_amdgpu_queue_request_retire(
  ...
  iree_amdgpu_scoped_atomic_fetch_or(
      &queue->scheduler->active_set.retire_bits, 1ul << entry->active_bit_index,
      iree_amdgpu_memory_order_release, iree_amdgpu_memory_scope_system);
...
iree_status_t iree_hal_amdgpu_queue_retire_entry(
  ...
  // Free the entry and return it to the pool for use by future submissions.
  iree_hal_amdgpu_queue_free_entry(queue, entry);
