...
static iree_status_t iree_hal_amdgpu_executable_load_modules(
  ...
  *out_handle = (hsa_executable_t){0};
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hsa_code_object_reader_create_from_memory(
          libhsa, image, flatbuffers_string_len(image), &code_object_reader));
...
static void iree_hal_amdgpu_executable_destroy(
  ...
  for (iree_host_size_t device_ordinal = 0;
    ...
    if (kernel_args) {
      IREE_IGNORE_ERROR(
          iree_hsa_amd_memory_pool_free(executable->libhsa, kernel_args));
    }
  }
