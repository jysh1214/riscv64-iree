...
iree_status_t iree_hal_amdgpu_physical_device_initialize(
  ...
  // Find the coarse device pool used for blocks.
  hsa_amd_memory_pool_t coarse_pool = {0};
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_amdgpu_find_coarse_global_memory_pool(
              &system->libhsa, device_agent, &coarse_pool));
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_amdgpu_block_pool_initialize(
        ...
        coarse_pool, host_allocator, &out_physical_device->block_pools.small);
  }
  if (iree_status_is_ok(status)) {
    status = iree_hal_amdgpu_block_pool_initialize(
        &system->libhsa, options->device_block_pools.large, device_agent,
        coarse_pool, host_allocator, &out_physical_device->block_pools.large);
  }
  if (iree_status_is_ok(status)) {
