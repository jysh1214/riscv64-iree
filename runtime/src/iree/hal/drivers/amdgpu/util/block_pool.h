...
// as part of management and may not even be host accessible.
//
...
// Releases the allocated blocks and returns them as a FIFO linked list.
// The arena will be reset and ready to allocate new blocks.
...
typedef union iree_hal_amdgpu_block_token_t {
  ...
  struct {
    ...
    // iree_hal_amdgpu_block_t pointer which is 64 byte aligned and thus only
    // needs 7 bytes for a full address. We could shave the top byte off if we
    // needed more space (up to 64k pages).
    uint64_t block : 56;
  };
} iree_hal_amdgpu_block_token_t;
static_assert(sizeof(iree_hal_amdgpu_block_token_t) == sizeof(uint64_t),
              "must match reserved space in device library");
...
// either touch the device-side memory or allocating additional host-side
...
// all acquired block storage will be wasted. That's rare for the intended usage
// though which is either ~64-128 byte allocations (most scheduler queue
...
// Requires that all allocations have been freed.
