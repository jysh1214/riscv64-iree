...
static void iree_hal_amdgpu_assign_cmd_query_ids(
  ...
  static const uint8_t required_ids[IREE_HAL_AMDGPU_DEVICE_CMD_MAX + 1] = {
      ...
      [IREE_HAL_AMDGPU_DEVICE_CMD_BRANCH] = ALL,
      [IREE_HAL_AMDGPU_DEVICE_CMD_RETURN] = ALL,
  };
...
typedef struct iree_hal_amdgpu_device_encoder_state_t {
  ...
  // block we encode. This wastes space but requires a compaction step during
...
static iree_status_t iree_hal_amdgpu_command_encoder_initialize(
  ...
  *out_encoder = NULL;
  ...
  // Capacties are defined as options or are derived from available pools.
...
// predictable 1-trip loop has no additional per-command overhead.
//
...
// the program than the cheap/garbagey ones: it's best to have deterministic
...
typedef struct iree_hal_amdgpu_command_buffer_t {
  ...
  // Retains references to any HAL reosurces used by the command buffer.
...
// Finializes command buffer recording by uploading metadata structures and
...
static iree_status_t iree_hal_amdgpu_command_buffer_finalize(
  ...
  for (iree_host_size_t i = 0; i < encoder->device_count; ++i) {
    ...
    // Take ownership of the command block linked list and associate metadata.
...
static iree_status_t iree_hal_amdgpu_command_buffer_begin_debug_group(
  ...
  uint64_t label_literal = (uint64_t) "todo_label_interning";
  ...
  for (iree_host_size_t i = 0; i < encoder->device_count; ++i) {
    ...
    cmd->src_loc = src_loc;
    cmd->label_literal = label_literal;
...
static iree_status_t iree_hal_amdgpu_command_buffer_update_buffer(
  ...
  for (iree_host_size_t i = 0; i < encoder->device_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        ...
        "embedding update data of %" PRIdsz "B", resolved_target_ref.length);
  }
...
static iree_status_t iree_hal_amdgpu_command_buffer_record_dispatch(
  ...
  // is loaded. These limits ensure we don't blow the host stack while
  // recording.
  IREE_ASSERT_LE(
      constants.data_length,
  ...
  // This is a microoptimization that mostly shows up in either smaller resource
  // sets or faster cleanup time (fewer redundant references to the same
  // executable).
  ...
  IREE_RETURN_IF_ERROR(iree_hal_amdgpu_command_encoder_append_cmd(
      ...
      host_kernel_args->kernarg_size, host_kernel_args->kernarg_alignment,
      (void***)&device_cmds, &kernarg_offset));
...
static iree_status_t iree_hal_amdgpu_command_buffer_dispatch_indirect(
  ...
  if (IREE_UNLIKELY(resolved_workgroups_ref.length < sizeof(uint32_t[3]))) {
    return iree_make_status(
        ...
        "uint32_t[3] (12B) but it resolved to %" PRId64 "B",
        resolved_workgroups_ref.length);
  }
  iree_hal_amdgpu_device_workgroup_count_t workgroup_count = {
      .ref =
          {
